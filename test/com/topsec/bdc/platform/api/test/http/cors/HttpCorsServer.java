 ...
 * under the License.
 */
package com.topsec.bdc.platform.api.test.http.cors;
...
/**
 * This example server aims to demonstrate <a href="http://www.w3.org/TR/cors/">Cross Origin Resource Sharing</a> (CORS) in Netty. It does not have a client like most of the other examples, but instead has a html page that is loaded to try out CORS support in a web brower.
 * <p>
 * 
 ...
 * <p>
 * 
 * The file {@code src/main/resources/cors/cors.html} contains a very basic example client which can be used to try out different configurations. For example, you can add custom headers to force a CORS preflight request to make the request fail. Then to enable a successful
 * request, configure the CorsHandler to allow that/those request headers.
 * 
 * <h2>Testing CORS</h2>
 ...
 * </pre>
 * 
 * If you inspect the headers being sent using your browser you'll see that the 'Origin' request header is {@code 'null'}. This is expected and happens when you load a file from the local file system. Netty can handle this by configuring the CorsHandler which is done in the
